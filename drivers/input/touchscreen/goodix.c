...
static int goodix_configure_dev(struct goodix_ts_data *ts)
{
	...
	/* Override keymap from device tree if provided */
	if (ts->client->dev.of_node) {
		u32 dt_keys[GOODIX_MAX_KEYS];

		memset(dt_keys, 0, sizeof(dt_keys));
		if (!of_property_read_u32_array(ts->client->dev.of_node,
						"goodix,keymap", dt_keys,
						GOODIX_MAX_KEYS)) {
			dev_info(&ts->client->dev, "Using keymap from device tree\n");
			for (i = 0; i < GOODIX_MAX_KEYS; ++i) {
				input_set_capability(ts->input_dev, EV_KEY,
						   ts->keymap[i]);
				ts->keymap[i] = dt_keys[i];
				input_set_capability(ts->input_dev, EV_KEY,
						   ts->keymap[i]);
			}
		}
