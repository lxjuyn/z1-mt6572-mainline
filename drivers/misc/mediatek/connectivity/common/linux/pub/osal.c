...
/* mainline 7.0: init_timer/timer->function/timer->data 已删,
 * 改用 timer_setup + container_of 反推 OSAL_TIMER 再调原回调传 data. */
static VOID osal_timer_thunk(struct timer_list *t)
{
...
{
    timer_setup(&pTimer->timer, osal_timer_thunk, 0);
    return 0;
}
...
{
    INT32 ret = 0;
    struct timespec64 ts;  /* mainline: do_gettimeofday 已删, 改用 ktime_get_real_ts64 */
