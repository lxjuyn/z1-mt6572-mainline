...
/* ---- mainline procfs/seq_file adapter for wmt_dbg ----
 * 原 wmt_dev_dbg_read 是老式 read_proc(char *page,char**start,off_t,int,int*,void*),
 * 原 wmt_dev_dbg_write 是老式 write_proc(struct file*,const char*,ulong,void*).
 * mainline(5.0+) 移除 create_proc_entry/read_proc/write_proc, 改 proc_create+file_operations.
 * adapter: seq_show 分配 page 调原 read; fops_write 用 simple_write 把 user buf 透给原 write.
 */
static int wmt_dbg_seq_show(struct seq_file *m, void *v)
{
    ...
    len = wmt_dev_dbg_read(page, &start, 0, PAGE_SIZE, &eof, NULL);
    if (len > 0) {
...
static int wmt_dbg_open(struct inode *inode, struct file *file)
{
    return single_open(file, wmt_dbg_seq_show, NULL);
}

static ssize_t wmt_dbg_fops_write(struct file *file, const char __user *buffer,
                                  size_t count, loff_t *off)
{
    /* 原 wmt_dev_dbg_write 签名 (file*,const char*,ulong,void*) 只用 buffer/count,
     * file/data 参数不被它真正读取, 透传 NULL data 安全. */
    return (ssize_t)wmt_dev_dbg_write(file, buffer, (unsigned long)count, NULL);
}

static const struct proc_ops wmt_dbg_fops = {
    .proc_open    = wmt_dbg_open,
    .proc_read    = seq_read,
    .proc_write   = wmt_dbg_fops_write,
    ...
    )
{
    ...
    *ppBufPtr = NULL;
    ...
    /* mainline: 不再需 get_current_cred/set_fs/凭据覆盖(模块以 root 跑,
     * filp_open 可读 root 文件). */
    fd = filp_open(pName, O_RDONLY, 0);
    if (!fd || IS_ERR(fd)) {
        WMT_ERR_FUNC("failed to open file %s (0x%p)\n", pName, fd);
        return -1;
    }

    file_len = (INT32)i_size_read(file_inode(fd));
    ...
    do {
        ...
        read_len = (INT32)kernel_read(fd, pBuf + padSzBuf, file_len, &pos);
    ...
    )
{
    ...
    /* mainline: 删整套 set_fs/get_ds/凭据覆盖——模块以 root 加载时
     * filp_open/kernel_read 已能读 firmware 文件, 无需跨 user/kernel addr space. */
    ...
    /* load patch file from fs (kernel root context, no set_fs needed) */
...
#if 0  // DISABLED mainline thermal Z1-noop
