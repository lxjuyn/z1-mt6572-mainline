...
static void stp_uart_tty_receive(struct tty_struct *tty, const u8 *data, const u8 *flags, size_t count)
{
 ...
 */
static int stp_uart_tty_ioctl(struct tty_struct *tty,
                    unsigned int cmd, unsigned long arg)
{
    ...
    case HCIUARTSETPROTO:
            /* mainline: tty->low_latency 字段已删, 仅留日志 */
            UART_DBG_FUNC("<!!> low_latency removed in mainline tty <!!>\n");
        break;
    default:
        UART_DBG_FUNC("<!!> n_tty_ioctl_helper <!!>\n");
        err = n_tty_ioctl_helper(tty, cmd, arg);  /* mainline 3参, 无 file */
        break;
    };
    UART_DBG_FUNC("%s <=\n", __FUNCTION__);
...
static __poll_t stp_uart_tty_poll(struct tty_struct *tty,
...
/* mainline: 提为文件静态变量, 供 exit 函数 unregister 使用 */
...
static int __init mtk_wcn_stp_uart_init(void)
{
    ...
    stp_uart_ldisc.num     = N_MTKSTP;   /* mainline: 编号放 ops->num, 无 magic */
    ...
    if ((err = tty_register_ldisc(&stp_uart_ldisc)))   /* mainline: 单参 */
    {
...
static void __exit mtk_wcn_stp_uart_exit(void)
{
    ...
    tty_unregister_ldisc(&stp_uart_ldisc);   /* mainline: 返回 void */
