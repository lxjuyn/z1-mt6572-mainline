...
/* ==== mainline 7.0 cfg80211 兼容别名(在 gl_os.h 已 include net/cfg80211.h 之后定义可见) ====
 * 3.4.67 vendor 源用 IEEE80211_BAND_* / WIPHY_FLAG_CUSTOM_REGULATORY,mainline 7.0 改了名/移了语义:
 *  - enum ieee80211_band 并入 enum nl80211_band,旧名整型值顺序不变(NL80211_BAND_2GHZ=0/5GHZ=1),
 *    wiphy->bands[] 索引与 ieee80211_channel.band 字段类型 enum nl80211_band 直接兼容。
 *  - WIPHY_FLAG_CUSTOM_REGULATORY 从 enum wiphy_flags 删除,语义搬到 wiphy->regulatory_flags
 *    的 REGULATORY_CUSTOM_REG=BIT(0),由 wiphy_apply_custom_regulatory() 注册时自动置位,driver 无需手设。
 *    此处别名占 0,位或运算无副作用,wiphy flags 留 WIPHY_FLAG_SUPPORTS_FW_ROAM 即可。
 */
...
#define WIPHY_FLAG_CUSTOM_REGULATORY 0
#endif
