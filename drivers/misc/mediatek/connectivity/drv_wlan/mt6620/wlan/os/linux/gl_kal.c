...
********************************************************************************
*/
#include <linux/timer.h>   /* mainline: for from_timer() 宏, timer_setup/timer_delete_sync 可见 */
#include "gl_os.h"
#include "gl_wext.h"
#include "precomp.h"
    ...
    )
{
    ...
    orgfsuid = __kuid_val(current->fsuid);
    orgfsgid = __kgid_val(current->fsgid);
    current->fsuid = GLOBAL_ROOT_UID; current->fsgid = GLOBAL_ROOT_GID;
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 35))
    struct cred *cred = (struct cred *) get_current_cred();
    orgfsuid = __kuid_val(cred->fsuid);
    orgfsgid = __kgid_val(cred->fsgid);
    cred->fsuid = GLOBAL_ROOT_UID; cred->fsgid = GLOBAL_ROOT_GID;
#else
...
error_open:
    /* restore */
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,29))
    current->fsuid = KUIDT_INIT(orgfsuid);
    current->fsgid = KGIDT_INIT(orgfsgid);
#else
    cred->fsuid = KUIDT_INIT(orgfsuid);
    cred->fsgid = KGIDT_INIT(orgfsgid);
    ...
    )
{
    ...
    DBGLOG(BOW, TRACE, ("\n"));
#endif
/* mainline: netif_rx_ni 已并入 netif_rx(进程/中断上下文都安全), 删除 in_interrupt 分支 */
    netif_rx(prSkb);
        ...
        if(prGlueInfo->fgIsRegistered == TRUE) {
            ...
            if(bss == NULL) {
                ...
                if(prBssDesc != NULL) {
                    bss = cfg80211_inform_bss(priv_to_wiphy(prGlueInfo),
                        prChannel,
                        CFG80211_BSS_FTYPE_BEACON,            /* mainline 新增 frame type */
                        arBssid,
                        0ULL,                                  /* TSF u32→u64 */
                        WLAN_CAPABILITY_ESS,
                        prBssDesc->u2BeaconInterval,            /* beacon interval */
                        prBssDesc->aucIEBuf,                    /* IE */
                        (size_t)prBssDesc->u2IELength,          /* IE Length u16→size_t */
            ...
            if(eStatus == WLAN_STATUS_MEDIA_CONNECT
                ...
                cfg80211_connect_result(prGlueInfo->prDevHandler,
                    ...
                    && prGlueInfo->eParamMediaStateIndicated == PARAM_MEDIA_STATE_CONNECTED) {
                /* mainline: cfg80211_roamed_bss 已移除, 改用 cfg80211_roamed + cfg80211_roam_info */
    ...
    case WLAN_STATUS_MEDIA_DISCONNECT:
        ...
        */
        ...
        if(prGlueInfo->fgIsRegistered == TRUE
                && prGlueInfo->eParamMediaStateIndicated == PARAM_MEDIA_STATE_CONNECTED) {
            /* CFG80211 Indication */
            /* mainline: cfg80211_disconnected 补 bool locally_generated 形参 */
            cfg80211_disconnected(prGlueInfo->prDevHandler, 0, NULL, 0, false, GFP_KERNEL);
        }
    ...
    )
{
    ...
    timer_setup(&(prGlueInfo->tickfn), kalTimeoutHandler, 0);
    (void)prTimerHandler; /* mainline: 单回调 kalTimeoutHandler */
}
...
/*----------------------------------------------------------------------------*/
VOID
kalTimeoutHandler (struct timer_list *t)
{
    /* mainline: timer 回调签名改, from_timer 取 prGlueInfo */
    P_GLUE_INFO_T prGlueInfo = container_of(t, GLUE_INFO_T, tickfn);
