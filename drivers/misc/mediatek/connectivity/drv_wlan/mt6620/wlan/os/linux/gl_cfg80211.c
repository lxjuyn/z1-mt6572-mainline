...
mtk_cfg80211_add_key (
    struct wiphy *wiphy,
    struct net_device *ndev,
    int link_id, /* mainline 7.0: netdev 后新增 link_id, mt6620 单链忽略 */
    u8 key_index,
    bool pairwise,
    const u8 *mac_addr,
...
mtk_cfg80211_get_key (
    struct wiphy *wiphy,
    struct net_device *ndev,
    int link_id, /* mainline 7.0: netdev 后新增 */
    u8 key_index,
    bool pairwise,
    const u8 *mac_addr,
    void *cookie,
...
mtk_cfg80211_get_station (
    struct wiphy *wiphy,
    struct net_device *ndev,
    const u8 *mac, /* mainline 7.0: mac const 化 */
...
mtk_cfg80211_remain_on_channel (
    struct wiphy *wiphy,
    struct wireless_dev *wdev, /* mainline 7.0: netdev→wdev */
...
mtk_cfg80211_mgmt_tx (
    struct wiphy *wiphy,
    struct wireless_dev *wdev, /* mainline 7.0: 12参→4参, netdev→wdev + params 打包所有字段 */
