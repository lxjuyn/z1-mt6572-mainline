    ...
    )
{
    ...
    do {
        ...
        prP2pBcnUpdateMsg = (P_MSG_P2P_BEACON_UPDATE_T)cnmMemAlloc(
                                                                prGlueInfo->prAdapter,
                                                                RAM_TYPE_MSG,
                                                                (sizeof(MSG_P2P_BEACON_UPDATE_T) + info->beacon.head_len + info->beacon.tail_len));
        ...
        if (info->beacon.head_len != 0) {
            kalMemCopy(pucBuffer, info->beacon.head, info->beacon.head_len);

            prP2pBcnUpdateMsg->u4BcnHdrLen = info->beacon.head_len;

            prP2pBcnUpdateMsg->pucBcnHdr = pucBuffer;

            pucBuffer = (PUINT_8)((UINT_32)pucBuffer + (UINT_32)info->beacon.head_len);
        }
        else {
            prP2pBcnUpdateMsg->u4BcnHdrLen = 0;
        ...
        if (info->beacon.tail_len != 0) {
            UINT_8 ucLen = info->beacon.tail_len;
            ...
            kalMemCopy(pucBuffer, info->beacon.tail, info->beacon.tail_len);
    ...
    )
{
    ...
    const u8 *buf;        /* mainline: 从 params 折叠 */
    size_t len;

    buf = params ? params->buf : NULL;
    len = params ? params->len : 0;
    (void)wdev;
    ...
    )
{
    ...
    const u8 *mac;   /* mainline: 从 station_del_parameters->mac 取 */
...
int mtk_p2p_cfg80211_testmode_cmd(
    IN struct wiphy *wiphy,
    IN struct wireless_dev *wdev, /* mainline 7.0: testmode_cmd 多 wdev 形参 */
    IN void *data,
    IN int len
    )
{
