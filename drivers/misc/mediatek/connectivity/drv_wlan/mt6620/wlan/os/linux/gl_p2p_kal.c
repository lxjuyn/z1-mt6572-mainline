    ...
    )
{
    ...
    do {
        ...
        cfg80211_ready_on_channel(prGlueInfo->prP2PInfo->prDevHandler->ieee80211_ptr, // mainline: wdev
                        u8SeqNum, //u64 cookie,
                        prIEEE80211ChnlStruct, //struct ieee80211_channel * chan,
                        u4Duration, //unsigned int duration,
                        GFP_KERNEL); //gfp_t gfp
    ...
    )
{
    ...
    do {
        ...
        cfg80211_remain_on_channel_expired(prGlueP2pInfo->prDevHandler->ieee80211_ptr, // mainline: wdev
    ...
    )
{
    ...
    do {
        ...
        cfg80211_put_bss(prGlueP2pInfo->wdev.wiphy, // mainline: 补 wiphy 首参
                         prCfg80211Bss);
    ...
    )
{
    ...
    do {
        ...
        cfg80211_mgmt_tx_status(prGlueP2pInfo->prDevHandler->ieee80211_ptr, // mainline: wdev
                        u8Cookie,
                        pucFrameBuf,
                        u4FrameLen,
                        fgIsAck,
...
#define DBG_P2P_MGMT_FRAME_INDICATION 0
    ...
    do {
        ...
        cfg80211_rx_mgmt(prGlueP2pInfo->prDevHandler->ieee80211_ptr, // mainline: wdev
                            i4Freq,
                            RCPI_TO_dBm(prSwRfb->prHifRxHdr->ucRcpi),
                            prSwRfb->pvHeader,
                            prSwRfb->u2PacketLen,
                            0);  /* mainline: 第6参是 u32 flags, 非 GFP. 0 = no flags */
#else
        cfg80211_rx_mgmt(prGlueP2pInfo->prDevHandler->ieee80211_ptr, // mainline: wdev
                            i4Freq,
                            RCPI_TO_dBm(prSwRfb->prHifRxHdr->ucRcpi),
    ...
    do {
        ...
        if (prP2pConnInfo) {
            ...
            cfg80211_disconnected(prGlueP2pInfo->prDevHandler, //struct net_device * dev,
                                    ...
                                    pucRxIEBuf,
                                    u2RxIELen,
                                    false,  /* mainline: 补 bool locally_generated */
                                    GFP_KERNEL);
        }
    ...
    )
{
    ...
    do {
        ...
        if (fgIsNew) {
            /* mainline: STATION_INFO_ASSOC_REQ_IES 已删, filled 不再用此 bit;
             * assoc_req_ies 仍由 cfg80211_new_sta 读取整结构. */
