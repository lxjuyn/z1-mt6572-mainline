...
/* forward declaration (must precede ops array which references .doit) */
...
/* operation definition (must precede family struct which references .ops) */
static struct genl_ops mtk_wifi_gnl_ops[] = {
    {
...
static struct genl_family mtk_wifi_gnl_family = {
    ...
    .ops        = mtk_wifi_gnl_ops,
    .n_ops      = ARRAY_SIZE(mtk_wifi_gnl_ops),
};
    ...
    )
{
    ...
    /* mainline: genl_register_ops 已移除. ops 内联进 family struct
     * (.ops/.n_ops), genl_register_family 一次性注册全部, else 里无需再注册 ops. */
