...
#if IS_ENABLED(CONFIG_MTK_PMIC_WRAP_Z1_WRITE_DENY)
enum pwrap_z1_diag_state {
	PWRAP_Z1_DIAG_UNAVAILABLE,
	PWRAP_Z1_DIAG_PROBING,
	PWRAP_Z1_DIAG_DEFERRED,
	PWRAP_Z1_DIAG_READY,
};

static DEFINE_MUTEX(pwrap_z1_diag_lock);
static struct pmic_wrapper *pwrap_z1_diag_wrp;
static enum pwrap_z1_diag_state pwrap_z1_diag_state;
static atomic64_t pwrap_z1_denied_writes = ATOMIC64_INIT(0);

static int pwrap_z1_diag_finish(bool target, int ret)
{
	if (!target)
		return ret;

	mutex_lock(&pwrap_z1_diag_lock);
	pwrap_z1_diag_state = !ret ? PWRAP_Z1_DIAG_READY :
		ret == -EPROBE_DEFER ? PWRAP_Z1_DIAG_DEFERRED :
		PWRAP_Z1_DIAG_UNAVAILABLE;
	mutex_unlock(&pwrap_z1_diag_lock);

	return ret;
}

static void pwrap_z1_diag_start(void)
{
	mutex_lock(&pwrap_z1_diag_lock);
	pwrap_z1_diag_state = PWRAP_Z1_DIAG_PROBING;
	mutex_unlock(&pwrap_z1_diag_lock);
}

int mtk_pmic_wrap_z1_probe_status(void)
{
	int ret;

	mutex_lock(&pwrap_z1_diag_lock);
	switch (pwrap_z1_diag_state) {
	case PWRAP_Z1_DIAG_PROBING:
	case PWRAP_Z1_DIAG_DEFERRED:
		ret = -EPROBE_DEFER;
		break;
	case PWRAP_Z1_DIAG_READY:
		ret = 0;
		break;
	default:
		ret = -ENODEV;
...
EXPORT_SYMBOL_GPL(mtk_pmic_wrap_z1_probe_status);

/**
 * mtk_pmic_wrap_z1_write_deny_active - report whether the Z1 gate is active
 *
 * This read-only diagnostic API is intended for bring-up validators. It
 * returns true after an MT6572/MT6323 wrapper on a jbo,z1 machine has enabled
 * the write-deny gate, and false otherwise.
 */
bool mtk_pmic_wrap_z1_write_deny_active(void)
{
	bool active;

	mutex_lock(&pwrap_z1_diag_lock);
	active = pwrap_z1_diag_wrp && pwrap_z1_diag_wrp->z1_write_deny &&
		 pwrap_z1_diag_state == PWRAP_Z1_DIAG_READY;
	mutex_unlock(&pwrap_z1_diag_lock);

	return active;
}
EXPORT_SYMBOL_GPL(mtk_pmic_wrap_z1_write_deny_active);

/**
 * mtk_pmic_wrap_z1_write_deny_count - return denied PMIC write attempts
 *
 * The counter is monotonic for the lifetime of the kernel and includes every
 * MT6323 serial write rejected before a WACS command was transmitted.
 */
u64 mtk_pmic_wrap_z1_write_deny_count(void)
{
	return atomic64_read(&pwrap_z1_denied_writes);
}
EXPORT_SYMBOL_GPL(mtk_pmic_wrap_z1_write_deny_count);

static void pwrap_z1_write_deny_unpublish(void *data)
{
	struct pmic_wrapper *wrp = data;

	mutex_lock(&pwrap_z1_diag_lock);
	if (pwrap_z1_diag_wrp == wrp) {
		pwrap_z1_diag_wrp = NULL;
		if (pwrap_z1_diag_state == PWRAP_Z1_DIAG_READY)
			pwrap_z1_diag_state = PWRAP_Z1_DIAG_UNAVAILABLE;
	}
	mutex_unlock(&pwrap_z1_diag_lock);
}

static int pwrap_z1_write_deny_publish(struct pmic_wrapper *wrp)
{
	int ret = 0;

	mutex_lock(&pwrap_z1_diag_lock);
	if (pwrap_z1_diag_wrp)
		ret = -EBUSY;
	else
		pwrap_z1_diag_wrp = wrp;
	mutex_unlock(&pwrap_z1_diag_lock);

	return ret;
}
#else
static inline int pwrap_z1_diag_finish(bool target, int ret)
{
	return ret;
}
#endif
...
#if IS_ENABLED(CONFIG_MTK_PMIC_WRAP_Z1_WRITE_DENY)
/**
 * mtk_pmic_wrap_z1_read - read an allowlisted MT6323 diagnostic register
 * @reg: MT6323 register address
 * @value: destination for the 16-bit PMIC register value
 *
 * This helper fails closed unless the JBO Z1 write-deny gate is active on an
 * MT6572 wrapper with an MT6323 slave. Only the three registers required by
 * the read-only MMC validator are accessible.
 */
int mtk_pmic_wrap_z1_read(u32 reg, u32 *value)
{
	...
	mutex_lock(&pwrap_z1_diag_lock);
	wrp = pwrap_z1_diag_wrp;
	if (!wrp || !wrp->z1_write_deny || !wrp->regmap ||
	    pwrap_z1_diag_state != PWRAP_Z1_DIAG_READY ||
	    wrp->master->type != PWRAP_MT6572 ||
	    wrp->slave->type != PMIC_MT6323 ||
	    !of_machine_is_compatible("jbo,z1")) {
		ret = -ENODEV;
		goto out_unlock;
	}

	ret = regmap_read(wrp->regmap, reg, value);

out_unlock:
	mutex_unlock(&pwrap_z1_diag_lock);
	return ret;
}
EXPORT_SYMBOL_GPL(mtk_pmic_wrap_z1_read);
#endif
...
{
#if IS_ENABLED(CONFIG_MTK_PMIC_WRAP_Z1_WRITE_DENY)
	if (unlikely(wrp->z1_write_deny)) {
		u64 count = atomic64_inc_return(&pwrap_z1_denied_writes);

		dev_err(wrp->dev,
			"Z1 diagnostic gate denied MT6323 write #%llu: reg=0x%04x value=0x%08x\n",
			(unsigned long long)count, adr, wdata);
		return -EPERM;
	}
#endif
...
static int pwrap_reset_spislave(struct pmic_wrapper *wrp)
{
	...
	if (unlikely(wrp->z1_write_deny)) {
		...
		dev_err(wrp->dev,
			"Z1 diagnostic gate denied manual SPI slave reset #%llu\n",
...
static int pwrap_probe(struct platform_device *pdev)
{
...
#if IS_ENABLED(CONFIG_MTK_PMIC_WRAP_Z1_WRITE_DENY)
	z1_diag = wrp->master->type == PWRAP_MT6572 &&
		  wrp->slave->type == PMIC_MT6323 &&
		  of_machine_is_compatible("jbo,z1");
	if (z1_diag)
		pwrap_z1_diag_start();
	...
	if (IS_ERR(wrp->base))
		return pwrap_z1_diag_finish(z1_diag, PTR_ERR(wrp->base));
	...
	if (HAS_CAP(wrp->master->caps, PWRAP_CAP_RESET)) {
		...
		if (IS_ERR(wrp->rstc)) {
			...
			return pwrap_z1_diag_finish(z1_diag, ret);
		}
	}
	...
	if (HAS_CAP(wrp->master->caps, PWRAP_CAP_BRIDGE)) {
		...
		if (IS_ERR(wrp->bridge_base))
			return pwrap_z1_diag_finish(z1_diag, PTR_ERR(wrp->bridge_base));
	...
	if (ret < 0)
		return pwrap_z1_diag_finish(z1_diag, dev_err_probe(wrp->dev, ret,
				     "failed to get clocks\n"));

	if (wrp->master->type == PWRAP_MT6572 &&
	    !pwrap_readl(wrp, PWRAP_INIT_DONE2)) {
		dev_err(wrp->dev,
			"MT6572 PMIC wrapper is not initialized; refusing cold init\n");
		return pwrap_z1_diag_finish(z1_diag, -ENODEV);
	}
	...
	if (wrp->master->type == PWRAP_MT6572 &&
	    wrp->slave->type == PMIC_MT6323 &&
	    of_machine_is_compatible("jbo,z1")) {
		wrp->z1_write_deny = true;
		ret = devm_add_action_or_reset(wrp->dev,
					       pwrap_z1_write_deny_unpublish, wrp);
		if (ret)
			return pwrap_z1_diag_finish(z1_diag, ret);
	}
#endif
	...
	if (irq < 0)
		return pwrap_z1_diag_finish(z1_diag, irq);
	...
	if (IS_ERR(wrp->regmap))
		return pwrap_z1_diag_finish(z1_diag, PTR_ERR(wrp->regmap));

#if IS_ENABLED(CONFIG_MTK_PMIC_WRAP_Z1_WRITE_DENY)
	if (wrp->z1_write_deny) {
		ret = pwrap_z1_write_deny_publish(wrp);
		if (ret)
			return pwrap_z1_diag_finish(z1_diag, ret);

		dev_warn(wrp->dev,
			 "Z1 diagnostic MT6323 write-deny gate active; PMIC reads remain enabled\n");
	}
#endif

#if IS_ENABLED(CONFIG_MTK_PMIC_WRAP_Z1_WRITE_DENY)
	if (wrp->z1_write_deny)
		ret = devm_of_platform_populate(wrp->dev);
	else
#endif
	...
	return pwrap_z1_diag_finish(z1_diag, 0);
}
