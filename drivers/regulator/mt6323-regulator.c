...
static int mt6323_regulator_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT6323_MAX_REGULATOR; i++) {
		...
		regulator_node = of_get_child_by_name(pdev->dev.of_node,
						      mt6323_regulators[i].desc.of_match);
		if (!regulator_node)
			continue;

		if (!of_device_is_available(regulator_node)) {
