...
static int regulator_late_cleanup(struct device *dev, void *data)
{
	...
	if (IS_ENABLED(CONFIG_REGULATOR_Z1_EMMC_CLEANUP_GUARD) &&
	    of_machine_is_compatible("jbo,z1") &&
	    (!strcmp(rdev_get_name(rdev), "vemc3v3") ||
	     !strcmp(rdev_get_name(rdev), "vio18"))) {
		rdev_warn(rdev,
			  "preserving Z1 eMMC regulator during late cleanup\n");
		return 0;
	}
